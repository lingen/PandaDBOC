...
-(NSString*)createTableSQL{
    ...
    [createTableSQL appendString:@"create table if not exists "];
    //表名
...
-(NSString*)createIndexSQL{
    ...
    if (_indexColumns) {
        for (OPDColumn* column in _indexColumns) {
            [indexString appendString:[NSString stringWithFormat:@"CREATE INDEX index_%@_%@ ON %@ (%@);",_tableName,column.name,_tableName,column.name]];
        }
    }
...
-(NSString*)p_primayKeyString{
    ...
    if (_primaryColumns) {
        ...
        for (int i=0; i< _primaryColumns.count; i++) {
            OPDColumn* column  = _primaryColumns[i];
        ...
        [primaryKey appendString:[NSString stringWithFormat:@"%@ integer PRIMARY KEY autoincrement", OPD_ID]];
    }
